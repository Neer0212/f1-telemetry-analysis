...
st.set_page_config(page_title="Session Deep Dive · F1 Analytics", page_icon="📊", layout="wide", initial_sidebar_state="collapsed")
...
from f1_analysis.core.lap_analysis import fastest_laps_by_driver, clean_lap_times, race_pace_summary, stint_summary
...
clicked, vals = control_panel([
    {"type":"number","label":"Year","key":"dd_year","default":2024,"min":2018,"max":2026},
    {"type":"text",  "label":"Grand Prix","key":"dd_gp","default":"Monza"},
    {"type":"select","label":"Session","key":"dd_session","default":"R","options":["R","Q","S","FP1","FP2","FP3"]},
    {"type":"text",  "label":"Drivers (space-separated)","key":"dd_drivers","default":"VER LEC NOR HAM RUS"},
], button_label="Run Analysis", cols_per_row=5)

if clicked:
    drivers = [d.strip().upper() for d in vals["dd_drivers"].split() if d.strip()]
    
    with st.spinner(f"Loading {vals['dd_year']} {vals['dd_gp']} ({vals['dd_session']})..."):
        apply_f1_style()
        try:
            session = load_session(vals["dd_year"], vals["dd_gp"], vals["dd_session"])
    ...
    gap = (p2["LapTimeSeconds"] - pole["LapTimeSeconds"]) if p2 is not None else 0

    metrics_row([
        {"label":"Event", "value":session.event["EventName"]},
        {"label":"Fastest Lap", "value":f"{pole['LapTimeSeconds']:.3f}s", "color":"accent"},
        {"label":"Gap P1→P2", "value":f"+{gap:.3f}s" if gap else "—", "color":"gold"},
        {"label":"Total Laps", "value":str(int(session.laps["LapNumber"].max()) if not session.laps.empty else 0)},
    ...
    fig1 = plot_lap_time_distribution(session, drivers)
    if fig1: 
    ...
    st.markdown('<div style="padding:5rem 2.5rem;text-align:center;font-family:\'Titillium Web\',sans-serif;font-size:0.8rem;text-transform:uppercase;letter-spacing:0.15em;color:#2A2A2A;">Configure parameters above and press Run Analysis</div>', unsafe_allow_html=True)